...
export type GitHubUserQuery = { user?: { login: string, name?: string | null, bio?: string | null, avatarUrl: any, url: any, company?: string | null, location?: string | null, followers: { totalCount: number }, following: { totalCount: number }, contributionsCollection: { contributionYears: Array<number>, totalCommitContributions: number, totalIssueContributions: number, totalPullRequestContributions: number, contributionCalendar: { totalContributions: number, weeks: Array<{ contributionDays: Array<{ contributionLevel: ContributionLevel, contributionCount: number }> }> } }, repositories: { totalCount: number, nodes?: Array<{ name: string, description?: string | null, url: any, stargazers: { totalCount: number }, watchers: { totalCount: number }, forks: { totalCount: number } } | null> | null } } | null };
...
/**
 * __useGitHubUserQuery__
 *
 * To run a query within a React component, call `useGitHubUserQuery` and pass it any options that fit your needs.
 * When your component renders, `useGitHubUserQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useGitHubUserQuery({
 *   variables: {
 *      username: // value for 'username'
 *   },
 * });
 */
export function useGitHubUserQuery(baseOptions: Apollo.QueryHookOptions<GitHubUserQuery, GitHubUserQueryVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<GitHubUserQuery, GitHubUserQueryVariables>(GitHubUserDocument, options);
      }
export function useGitHubUserLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<GitHubUserQuery, GitHubUserQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<GitHubUserQuery, GitHubUserQueryVariables>(GitHubUserDocument, options);
        }
export type GitHubUserQueryHookResult = ReturnType<typeof useGitHubUserQuery>;
export type GitHubUserLazyQueryHookResult = ReturnType<typeof useGitHubUserLazyQuery>;
export type GitHubUserQueryResult = Apollo.QueryResult<GitHubUserQuery, GitHubUserQueryVariables>;
export function refetchGitHubUserQuery(variables: GitHubUserQueryVariables) {
      return { query: GitHubUserDocument, variables: variables }
    }